...
保留原几何数据与状态，在引擎中做以下局部改动：`tables.js` 从播放清单排除用户指定的眼型 7、8，平静待机固定基础眼型 0，保留呼吸与眨眼；完整表情由场景层低频触发。倾听固定眼型与视线，`pose.js` 用状态经过时间播放一次约 550ms 的点头，之后仅轻微呼吸与眨眼；`character.js` 提供 `autoTricks` 控制，并在减少动态效果时固定绘制姿态，切换状态时清理粒子；`fx.js` 清理粒子时同时重置旋转速度记录，避免中止后的彩带残留。本站已撤下进度环，原始动作库仍保留。场景选择与回答展示仍位于 vendor 之外。
...
首次入场和休眠后唤醒复用原 `spawning` 状态与 `fx.js` 的 `paintGather` 粒子聚拢；入场时从已收拢的小形态起播，聚拢结束才展开，避免先缩小再变大。休眠途中被操作打断时，从当前大小衔接聚拢。此前本地增加的 `powering-up` 反向休眠状态已移除。
