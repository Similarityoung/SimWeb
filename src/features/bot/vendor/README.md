...
保留原几何数据与状态，在引擎中做以下局部改动：`tables.js` 从播放清单排除用户指定的眼型 7、8，平静待机固定基础眼型 0，保留呼吸与眨眼；完整表情由场景层低频触发；`character.js` 提供 `autoTricks` 控制，并在减少动态效果时固定绘制姿态，切换状态时清理粒子；`fx.js` 清理粒子时同时重置旋转速度记录，避免中止后的彩带残留。本站已撤下进度环，原始动作库仍保留。场景选择与回答展示仍位于 vendor 之外。
