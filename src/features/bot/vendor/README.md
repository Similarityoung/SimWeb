...
首次入场和休眠后唤醒复用原 `spawning` 状态与 `fx.js` 的 `paintGather` 粒子聚拢；入场时从已收拢的小形态起播，聚拢结束才展开，避免先缩小再变大。休眠途中被操作打断时，从当前大小衔接聚拢；聚拢粒子的可见度独立于身体收缩进度。身体尚大时，聚拢粒子会暂时绘于身体前方并使用本站强调色，确保刚进入休眠就唤醒也能看到；完整休眠后的苏醒仍沿用原深色粒子。此前本地增加的 `powering-up` 反向休眠状态已移除。
