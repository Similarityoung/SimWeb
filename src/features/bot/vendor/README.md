...
完成反馈使用单次 `celebrate`：`tricks.js` 的 `spinHop` 按时间戳先转一圈、再轻跳一次；`character.js` 在落地时释放 14 个低速粒子，消费后不重复，旋转阶段不产生外围彩带。`pose.js` 取消此状态的额外上下摆动，`fx.js` 为实际粒子添加观测标记。时长、优先级与中止仍归外部 Bot 场景控制器。
